...
fn test_trace_byte_by_byte() {
    ...
    read_and_track!(buffer.read_bool().unwrap(), 1, "outer_present");
    ...
    // Interval
    println!("\n=== Interval ===");
    let chr1 = string_type.read(&mut buffer).unwrap();
    let chr1_str = chr1.as_string().unwrap();
    println!("[{:04x}] chr_start (len={}) = {:?}", offset, chr1_str.len() + 1, chr1_str);
    offset += chr1_str.len() + 1;

    read_and_track!(buffer.read_i32().unwrap(), 4, "pos_start");

    let chr2 = string_type.read(&mut buffer).unwrap();
    let chr2_str = chr2.as_string().unwrap();
    println!("[{:04x}] chr_end (len={}) = {:?}", offset, chr2_str.len() + 1, chr2_str);
    offset += chr2_str.len() + 1;

    read_and_track!(buffer.read_i32().unwrap(), 4, "pos_end");
    read_and_track!(buffer.read_bool().unwrap(), 1, "inc_start");
    read_and_track!(buffer.read_bool().unwrap(), 1, "inc_end");
    ...
    let gene_ver = string_type.read(&mut buffer).unwrap();
    ...
    println!("\n=== Numeric Fields ===");
    read_and_track!(buffer.read_i32().unwrap(), 4, "start (i32)");
    read_and_track!(buffer.read_i32().unwrap(), 4, "stop (i32)");
    ...
    buffer.read_exact(&mut next_bytes).unwrap();
