...
fn test_decode_all_primitive_fields() {
    ...
    // === INTERVAL ===
    println!("\n--- Interval ---");
    let chr_start = string_type.read(&mut zstd).unwrap();
    let pos_start = zstd.read_i32().unwrap();
    let chr_end = string_type.read(&mut zstd).unwrap();
    let pos_end = zstd.read_i32().unwrap();
    ...
    // === GENE_VERSION ===
    let gene_version = string_type.read(&mut zstd).unwrap();
    ...
    // === START (Int32) ===
    let start = zstd.read_i32().unwrap();
    println!("Start: {}", start);

    // === STOP (Int32) ===
    let stop = zstd.read_i32().unwrap();
    println!("Stop: {}", stop);
...
fn test_decode_with_array_inspection() {
    ...
    string_type.read(&mut zstd).unwrap(); // chr start
    zstd.read_i32().unwrap(); // pos start
    string_type.read(&mut zstd).unwrap(); // chr end
    zstd.read_i32().unwrap(); // pos end
    zstd.read_bool().unwrap(); // inc start
    zstd.read_bool().unwrap(); // inc end
    ...
    let array_length = zstd.read_i32().unwrap();
    ...
    if array_length > 0 && array_length < 1000 {
        ...
        let feature_type = string_type.read(&mut zstd).unwrap();
        println!("  feature_type: {}", feature_type.as_string().unwrap());

        let exon_start = zstd.read_i32().unwrap();
        println!("  start: {}", exon_start);

        let exon_stop = zstd.read_i32().unwrap();
        println!("  stop: {}", exon_stop);

        let exon_xstart = zstd.read_i64().unwrap();
        println!("  xstart: {}", exon_xstart);

        let exon_xstop = zstd.read_i64().unwrap();
