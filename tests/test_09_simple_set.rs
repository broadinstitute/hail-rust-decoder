...
#[test]
fn test_09_simple_set() {
    let metadata_path = "test_hail_data/09_simple_set.ht/rows/metadata.json.gz";
    ...
    let parts_dir = "test_hail_data/09_simple_set.ht/rows/parts";
    ...
    for part_file in &entries {
        ...
        if let EncodedValue::Struct(fields) = result {
            ...
            let tags = fields.iter().find(|(n, _)| n == "tags").map(|(_, v)| v);
            ...
            // Check tags (Set encoded as Array)
            if let Some(EncodedValue::Array(tags_array)) = tags {
                println!("tags (set): {:?}", tags_array);

                // Verify based on actual id
                match id_str.as_str() {
                    "test1" => {
                        assert_eq!(tags_array.len(), 3);
                        // Note: Sets are sorted, so {1, 2, 3}
                        assert_eq!(tags_array[0], EncodedValue::Int32(1));
                        assert_eq!(tags_array[1], EncodedValue::Int32(2));
                        assert_eq!(tags_array[2], EncodedValue::Int32(3));
                    },
                    "test2" => {
                        assert_eq!(tags_array.len(), 2);
                        assert_eq!(tags_array[0], EncodedValue::Int32(4));
                        assert_eq!(tags_array[1], EncodedValue::Int32(5));
                    },
                    "test3" => {
                        assert_eq!(tags_array.len(), 1);
                        assert_eq!(tags_array[0], EncodedValue::Int32(6));
                    },
