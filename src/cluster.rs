...
pub fn show_cluster(config: &Config, name: &str) -> Result<()> {
    let cluster = config
        ...
        .ok_or_else(|| hail_decoder::HailError::InvalidFormat(format!("Unknown cluster: {}", name)))?;
...
pub fn deploy_cluster(config: &Config, name: &str, tag: &str, backend_only: bool) -> Result<()> {
    ...
    let project = config
        ...
        .project
        .as_deref()
        .ok_or_else(|| hail_decoder::HailError::Config("No project configured in defaults".into()))?;
